...
const setReminder = (time, userId, ctx) => {
  ...
  currentTime.setHours(currentTime.getUTCHours() + DESIRED_GMT);
  ...
  store[userId].reminder = setTimeout(async () => {
    await ctx.reply(config.ru.reminder.notification.msg);
  ...
  const userId = ctx.message.from.id;
	if (store[userId] && store[userId].state === 'waiting_for_time'){
		...
		if (isCorrectTime(userInput)) {
			...
			await ctx.reply(`Принято, в ${ctx.message.text} вам придет напоминание о необходимости принять лекарства`);
			store[userId].state = null;
		} else {
			await ctx.reply('Ошибка: некорректный формат даты. Пожалуйста, используйте формат, типа: 9:00');
		}
	}
	next();
});
	...
	for (const lang in menu) {
		...
		for (const key in langMenu) {
			...
			handlers[btnText] = async (ctx) => {
				...
				} else if (btnText === langMenu.psycho) {
					await ctx.reply(config.psyhoContact);
				} else {
