...
const setReminder = (time) => {
  ...
  if (currentHours < hours || (currentHours === hours && currentMinutes < minutes)) {
    delay = (hours - currentHours) * 3600 * 1000 + (minutes - currentMinutes) * 60 * 1000;
  } else {
    delay = (hours + 24 - currentHours) * 3600 * 1000 + (minutes - currentMinutes) * 60 * 1000;
  }
...
const createHandlerReminder = (lang) => {
	...
	handlers['oneTime'] = async (ctx) => {
		await ctx.reply(config.ru.reminder.whatTime.msg);
		store[ctx.update.callback_query.from.id] = { state: 'waiting_for_time' };
	};
};

bot.on('text', (ctx, next) => {
	if (store[ctx.message.from.id] && store[ctx.message.from.id].state === 'waiting_for_time'){
    ...
    if (isCorrectTime(userInput)) {
      ...
      store[ctx.message.from.id].state = null;
    } else {
...
generateKeyboards().then(keyboards => {
	handlers['/start'] = async (ctx) => {
		await ctx.reply(config.city, keyboards.cities);
	};
